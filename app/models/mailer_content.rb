class MailerContent < ActiveRecord::Base
  ...
  scope :next_acts_of_service, Proc.new { |user_id, limit = 1| where('category = "Acts of Service" AND workflow_state = "not_done" AND user_id = ?', user_id).order("user_id ASC").limit(limit) }

  scope :next_physical_touch, Proc.new { |user_id, limit = 1| where('category = "Physical Touch" AND workflow_state = "not_done" AND user_id = ?', user_id).order("user_id ASC").limit(limit) }

  scope :next_tangible_gift, Proc.new { |user_id, limit = 1| where('category = "Tangible Gifts" AND workflow_state = "not_done" AND user_id = ?', user_id).order("user_id ASC").limit(limit) }
